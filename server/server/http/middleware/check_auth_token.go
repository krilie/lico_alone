	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if err == jwt.ErrIatTime {
				...
				c.AbortWithStatusJSON(401, model.NewRetFromErr(errs.NewUnauthorized().WithMsg("token expired")))
				return
			} else {
				c.AbortWithStatusJSON(500, model.NewRetFromErr(errs.NewInternal().WithMsg(err.Error())))
				return
			}
		} else {
