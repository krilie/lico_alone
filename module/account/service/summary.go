...
// 统计一个时区的信息
func (Account) GetTimeZoneSummary(ctx context.Context, userId string, timeStart, timeEnd time.Time) (*model.AccountSummary, error) {
	// 参数格式检查
	...
	summary.Accounts = make([]model.AccountItem, 0, 4)
	summary.Bills = make([]model.BillItem, 0, 4)
	// 查询账户
	accounts := make([]model.Account, 0, 4)
	if e := dao.Db.Find(accounts, "user_id=?", userId).Error; e != nil {
		return nil, errs.ErrInternal.NewWithMsg(e.Error())
	}
	// 帐户统计 accounts
	for _, v := range accounts {
		details := make([]model.BillDetail, 0, 4)
		if e := dao.Db.Where("create_time between ? and ? and is_valid=true and account_id=?", timeStart, timeEnd, v.Id).Find(details).Error; e != nil {
			return nil, errs.ErrInternal.NewWithMsg(e.Error())
		}
		item := model.AccountItem{Id: v.Id, Name: v.Name, Num: v.Num, Credit: v.Credit, Debit: v.Debit, Amount: decimal.Zero, NowBalance: v.Balance}
	...
	if e := dao.Db.Where("create_time between ? and ? and user_id=?", timeStart, timeEnd, userId).Find(bills).Error; e != nil {
		return nil, errs.ErrInternal.NewWithMsg(e.Error())
	}
	for _, v := range bills {
		tempBill := model.BillItem{Id: v.Id, CreateTime: v.CreateTime, Note: v.Note, Amount: v.Amount, Image: v.Image, IsValid: v.IsValid}
